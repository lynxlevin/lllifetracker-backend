...
async fn happy_path_time_span_to_count() -> Result<(), DbErr> {
    ...
    let req = test::TestRequest::put()
        ...
        .set_json(ActionTrackTypeConversionRequest { track_type: ActionTrackType::Count })
        .to_request();
...
async fn happy_path_count_to_time_span() -> Result<(), DbErr> {
    ...
    let req = test::TestRequest::put()
        .uri(&format!("/api/actions/{}/track_type", action.id))
        .set_json(ActionTrackTypeConversionRequest { track_type: ActionTrackType::TimeSpan })
        .to_request();
    req.extensions_mut().insert(user.clone());
