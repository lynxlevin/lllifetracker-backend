use actix_web::{http, test, HttpMessage};
use sea_orm::{ActiveModelTrait, DbErr, EntityTrait};
use use_cases::my_way::directions::types::DirectionBulkUpdateOrderingRequest;

use crate::utils::Connections;

use super::super::utils::init_app;
use common::factory::{self, *};
use entities::direction;

#[actix_web::test]
async fn happy_path() -> Result<(), DbErr> {
    let Connections { app, db, .. } = init_app().await?;
    let user = factory::user().insert(&db.db).await?;
    let directions = create_directions(
        vec![
            DirectionParam { name: "direction_0", ..Default::default() },
            DirectionParam { name: "direction_1", ..Default::default() },
            DirectionParam { name: "direction_2", ..Default::default() },
        ],
        &user,
        &db,
    )
    .await?;
    let direction_0 = directions.get("direction_0").unwrap();
    let direction_1 = directions.get("direction_1").unwrap();
    let direction_2 = directions.get("direction_2").unwrap();

    let req = test::TestRequest::put()
        .uri("/api/directions/bulk_update_ordering")
        .set_json(DirectionBulkUpdateOrderingRequest { ordering: vec![direction_0.id, direction_1.id] })
        .to_request();
    req.extensions_mut().insert(user.clone());

    let res = test::call_service(&app, req).await;
    assert_eq!(res.status(), http::StatusCode::OK);

    let category_in_db_0 = direction::Entity::find_by_id(direction_0.id)
        .one(&db.db)
        .await?
        .unwrap();
    assert_eq!(category_in_db_0.ordering, Some(1));

    let category_in_db_1 = direction::Entity::find_by_id(direction_1.id)
        .one(&db.db)
        .await?
        .unwrap();
    assert_eq!(category_in_db_1.ordering, Some(2));

    let direction_in_db_2 = direction::Entity::find_by_id(direction_2.id)
        .one(&db.db)
        .await?
        .unwrap();
    assert_eq!(direction_in_db_2.ordering, None);

    Ok(())
}
