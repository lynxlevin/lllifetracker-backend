...
async fn login_to_get_me_to_logout() -> Result<(), DbErr> {
    ...
    let login_req = test::TestRequest::post()
        .uri("/api/users/login")
        .set_json(LoginRequest { email: user.email.to_string(), password: password.to_string() })
        .to_request();
    ...
    let session_set_cookie = set_cookie_header.find(|sc| sc.to_str().unwrap().starts_with("sessionId="));
    assert!(session_set_cookie.is_some());
    let session_cookie =
        Cookie::parse(urlencoding::decode(session_set_cookie.unwrap().to_str().unwrap()).unwrap()).unwrap();
