...
async fn login_user(
    ...
    match redis_pool.get().await {
        Ok(ref mut redis_con) => match validate_request_count(redis_con, &req_user.email, &settings).await {
            Ok((login_request_count_key, login_request_count)) => {
                    ...
                    .await
                {
                    Ok(user) => match user {
                        Some(user) => {
                            match verify_password(&user.password, req_user.password.clone().as_bytes()) {
                                Ok(_) => {
                                    tracing::event!(target: "backend", tracing::Level::INFO, "User logged in successfully.");
                                    if let Err(e) = redis_con.del::<String, String>(login_request_count_key).await
                                    {
            ...
            Err(_) => {
                HttpResponse::Unauthorized().json("Your account is temporarily locked. Please wait for 1 hour.")
            }
        },
        Err(e) => response_500(e),
    }
...
async fn increment_login_request_count(
    ...
    if let Err(e) = redis_con
        .set_options::<String, u64, String>(
            ...
            SetOptions::default()
                .with_expiration(SetExpiry::EX(settings.application.login_attempts_cool_time_seconds)),
        )
        .await
    {
