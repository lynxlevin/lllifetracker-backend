...
#[tracing::instrument(
    name = "Connecting an action to an desired_state",
    skip(db, user, path_param)
)]
#[post("/{desired_state_id}/actions/{action_id}/connection")]
    ...
    match user {
        Some(user) => {
            ...
            match validate_ownership(&db, user.id, &path_param).await {
                Ok(_) => {
                    match DesiredStateMutation::connect_action(
                        &db,
                    ...
                    .await
                    {
                        Ok(_) => HttpResponse::Ok().json(types::SuccessResponse {
                            message: format!(
                                "Successfully connected desired_state: {} with action: {}",
...
mod tests {
    ...
    async fn init_app(
        ...
        test::init_service(App::new().service(connect_action).app_data(Data::new(db))).await
    }
